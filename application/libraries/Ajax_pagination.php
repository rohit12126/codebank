...
class Ajax_pagination{
    ...
    var $next_link       = '<span class="next-link-icon"><i class="icofont icofont-thin-right"></i></span>';
    var $prev_link       = '<span class="prev-link-icon"><i class="icofont icofont-thin-left"></i></span>';
    var $last_link       = 'Last';
    var $uri_segment     = 3;
    ...
    function create_links(){
        ...
        if ($num_pages == 1){
            $info = $this->full_tag_open .'<span class="showing">Showing : ' . $this->total_rows.'</span>'.$this->full_tag_close;
            return $info;
        }

        // Determine the current page number.        
        $CI =& get_instance();    
        if($this->cur_page_set)
        {
            $this->cur_page=$this->cur_page_set;
        ...
        if ($this->show_count){
            if($this->cur_page_set)
                $curr_offset =$this->cur_page_set;
            else
                $curr_offset = $CI->uri->segment($this->uri_segment);
           
            $info = 'Showing ' . ( $curr_offset + 1 ) . ' - ' ;
            ...
            $info .= ' products of ' . $this->total_rows . ' products';
            $output .= $info;
        }
